...
class Encrypt:
    ...
    def encrypt_message(self, message, key):
        bytess = message.encode()                                    #Decodes message string into bytes
        i_int = int.from_bytes(bytess, byteorder='big')            #Integer from bytes
...
class Decrypt:
    ...
    def decrypt_message(self, message, size, key):
        ...
        bytess = self.int_to_bytes(decrypted_msg, size)              #Bytes from integer
        text = bytess.decode()                                       #Decodes bytes into text
        return text
